...
On Windows, the script auto-detects and switches to foreground mode (which blocks the tool call). Use `run_in_background: true` on the pwsh tool call so the server survives across conversation turns, then read `$STATE_DIR/server-info` on the next turn to get the URL and port. The launcher is a `.sh` script, so invoke it via bash (on Windows, call Git Bash's `bash.exe` from the pwsh tool).
